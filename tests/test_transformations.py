...
def test_adstock_invalid_lam():
    ...
    with pytest.raises(ValueError):
        adstock(x, 1.5)  # invalid decay

def test_adstock_non_numpy_input():
    with pytest.raises(TypeError):
        adstock([10, 20], 0.5)  # list instead of np.ndarray
...
def test_hill_basic_behavior():
    ...
    out = hill_saturation(x, theta=50, gamma=2)
...
def test_hill_invalid_inputs():
    x = np.array([10, 20])
    with pytest.raises(ValueError):
        hill_saturation(x, theta=-10, gamma=2)  # invalid theta
    with pytest.raises(ValueError):
        hill_saturation(x, theta=10, gamma=0)   # invalid gamma

def test_hill_negative_x():
    x = np.array([-5, 10])
    with pytest.raises(ValueError):
        hill_saturation(x, theta=50, gamma=2)

def test_hill_non_numpy_input():
    with pytest.raises(TypeError):
        hill_saturation([10, 20], theta=50, gamma=2)
